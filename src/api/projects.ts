...
export const clearProjectTrash = async (): Promise<void> => {
  try {
    await apiRequest("/projects/clear-trash", {
      method: "DELETE",
    });
  } catch (error) {
    // Fallback for mock mode / json-server where custom DELETE /projects/clear-trash is not available
    const { projects } = await getProjects();
    const deletedProjects = projects.filter((p) => p.isDeleted);
    await Promise.all(
      deletedProjects.map((p) =>
        apiRequest(`/projects/${p.id}`, {
          method: "DELETE",
        }),
      ),
    );
...
const resolveProjectTeam = async (
