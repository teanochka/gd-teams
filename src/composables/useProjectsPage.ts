...
export function useProjectsPage() {
  ...
  const { activeSection, error, isLoading, projects, teamsWithCounts } =
  ...
  const activeTitle = computed(() => {
    ...
    return (
      teamsWithCounts.value.find((team) => team.id === activeSection.value)
        ?.name ?? "Все проекты"
    );
  });
  ...
  return {
    ...
    clearTrash: projectsStore.clearTrash,
  };
}
