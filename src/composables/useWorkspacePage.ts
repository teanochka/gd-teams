...
export function useWorkspacePage() {
  ...
  const sidebarTags = computed(() =>
    tags.value.map((tag) => ({ id: tag.id, name: tag.name, color: tag.color })),
  );
  ...
  const parsedSearchQuery = computed(() => {
    ...
    const regex =
      /(name:|tags:|type:|created_by:|date:)?\s*([^:]+?)(?=\s*(name:|tags:|type:|created_by:|date:)|$)/g;
    let match;
    let hasTokens = false;
    while ((match = regex.exec(query)) !== null) {
      ...
      if (key === "name:") {
        ...
        result.createdBy = val.toLowerCase();
        hasTokens = true;
      } else if (key === "date:") {
        result.date = val.toLowerCase();
        hasTokens = true;
      } else if (!key && !hasTokens) {
        result.name = val.toLowerCase();
      }
    }
    return result;
  ...
  const visibleItems = computed(() => {
    ...
    const filtered = currentItems.value.filter((node) => {
      if (
        selectedTags.length &&
        !node.tags.some((tag) => selectedTags.includes(tag.id))
      ) {
        return false;
      }
      ...
      if (
        parsedQuery.name &&
        !node.title.toLowerCase().includes(parsedQuery.name)
      )
        return false;
      if (
        parsedQuery.type &&
        !typeLabels[node.type].toLowerCase().includes(parsedQuery.type) &&
        !node.type.toLowerCase().includes(parsedQuery.type)
      )
        return false;
      if (
        parsedQuery.createdBy &&
        !node.createdBy.toLowerCase().includes(parsedQuery.createdBy)
      )
        return false;
      if (parsedQuery.tags.length > 0) {
        const nodeTags = node.tags.map((t) => t.name.toLowerCase());
        const hasAllTags = parsedQuery.tags.every((pt) =>
          nodeTags.some((nt) => nt.includes(pt)),
        );
    ...
    const sortedItems = [...filtered]
      .sort((left, right) => {
        ...
        if (field === "type") {
          return (
            compareText(typeLabels[left.type], typeLabels[right.type]) *
            direction
          );
        }
  ...
  const getSelectableNodeElements = () => {
    ...
    return Array.from(
      contentRef.value.querySelectorAll<HTMLElement>("[data-node-id]"),
    ).filter((element) =>
      selectableIds.has(String(element.dataset.nodeId ?? "")),
    );
  };

  const getIntersectedNodeIds = (
    left: number,
    top: number,
    right: number,
    bottom: number,
  ...
  const finishItemName = async () => {
    ...
    if (editingMode.value === "create") {
      ...
      try {
        const nextTitle =
          draftItemName.value.trim() || creatingNodeDraft.value.title;
