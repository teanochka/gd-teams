...
export const toCssSize = (value: number | string | undefined, fallback: string) => {
  if (value === undefined) {
    return fallback
  }
...
export const getBlockStyle = (
  ...
  const style: CSSProperties = {
    backgroundColor: element.backgroundColor ?? options.backgroundColor ?? '#ffffff',
    borderColor: element.borderColor ?? options.borderColor ?? '#6b7280',
    borderWidth: `${element.borderWidth ?? 1}px`,
    borderRadius: toCssSize(element.borderRadius, toCssSize(options.borderRadius, '0px')),
  }
