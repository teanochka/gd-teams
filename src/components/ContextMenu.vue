...
const props = defineProps<{
  options: { label: string; action: () => void; icon?: any; disabled?: boolean; divider?: boolean }[]
}>()
...
<template>
  <div v-if="visible" ref="menuRef" class="context-menu" :style="{ top: `${position.y}px`, left: `${position.x}px` }">
    <template v-for="(opt, idx) in options" :key="idx">
      <div v-if="opt.divider" class="menu-divider"></div>
      <button v-else class="menu-item" :disabled="opt.disabled" @click="handleAction(opt.action)">
