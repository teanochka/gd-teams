...
const autocompleteOptions = computed(() => {
  ...
  if (token === "tags:") {
    ...
    return props.tags
      .filter(
        (t) =>
          t.name.toLowerCase().startsWith(lastTag) &&
          !currentVal.includes(t.name),
      )
  ...
  if (token === "type:") {
    ...
    return types
      .filter((t) => t.startsWith(currentVal.toLowerCase()))
      .map((t) => ({ label: t, value: t }));
  }
  if (token === "created_by:") {
    return props.users
      .filter((u) => u.toLowerCase().startsWith(currentVal.toLowerCase()))
...
<template>
  <div class="workspace-search-container" ref="dropdownRef">
    ...
    <div
      v-if="showDropdown && (search === '' || autocompleteOptions.length > 0)"
