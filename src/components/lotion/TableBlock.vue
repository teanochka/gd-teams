...
function ensureTable() {
  ...
  if (
    ...
    nextColumnWidths.some((width, index) => width !== existingTable.columnWidths[index])
  ) {
  ...
  if (
    ...
    nextRows.some((row: string[], rowIndex: number) =>
  ...
  if (
    ...
    nextRowHeights.some((height, index) => height !== existingTable.rowHeights[index])
  ) {
...
function isColumnSelected(column: number) {
  return menu.value?.type === 'column' && menu.value.index === column
}

function isRowSelected(row: number) {
  return menu.value?.type === 'row' && menu.value.index === row
}
...
function addRow(index = rowCount.value) {
  table.value.rows.splice(index, 0, Array.from({ length: columnCount.value }, () => ''))
...
function duplicateRow(index: number) {
  const row = table.value.rows[index] ?? Array.from({ length: columnCount.value }, () => '')
...
function startResize(type: 'row' | 'column', index: number, event: PointerEvent) {
  if (props.readonly) {
    return
  }
...
function handleResizeMove(event: PointerEvent) {
  ...
  const currentPosition = state.type === 'column' ? event.clientX : event.clientY
  ...
  table.value.rowHeights[state.index] = Math.max(minRowHeight, state.startSize + delta)
}
...
<template>
  <div class="table-block" :data-table-block="block.id" @click.self="menu = null">
    <div class="table-scroll">
      <div class="table-stage" :style="{ width: `${tableWidth + 76}px` }">
        <div class="table-top-controls" :style="{ width: `${tableWidth + 40}px` }">
          <div class="table-corner" />
          <div
            v-for="(_, columnIndex) in table.columnWidths"
            :key="`column-control-${columnIndex}`"
            class="column-control"
            :class="{ active: selectedCell?.column === columnIndex || isColumnSelected(columnIndex) }"
            :style="{ width: `${table.columnWidths[columnIndex]}px` }"
          >
            <button
              v-if="selectedCell?.column === columnIndex || isColumnSelected(columnIndex)"
        ...
        <div class="table-body">
          ...
          >
            <div
              class="row-control"
              :class="{ active: selectedCell?.row === rowIndex || isRowSelected(rowIndex) }"
            >
              <button
                v-if="selectedCell?.row === rowIndex || isRowSelected(rowIndex)"
                class="axis-menu-button"
                type="button"
    ...
    >
      ...
      <button type="button" @click="runMenuAction('duplicate')">Дублировать</button>
      <button type="button" @click="runMenuAction('clear')">Очистить</button>
      <button class="danger" type="button" @click="runMenuAction('delete')">Удалить</button>
    </div>
  </div>
</template>
