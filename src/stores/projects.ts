...
import {
  ...
  updateProjectRole,
  deleteProjectRole,
  clearProjectTrash as apiClearProjectTrash,
} from "@/api/projects";
import type {
...
export const useProjectsStore = defineStore("projects", () => {
  ...
  const teamsWithCounts = computed(() => {
    return teams.value.map((team) => ({
      ...
      count: projects.value.filter(
        (project) => project.teamId === team.id && !project.isDeleted,
      ).length,
    }));
  });
  ...
  function ensureProjectTeam(project: Project) {
    if (!teams.value.some((team) => team.id === project.teamId)) {
      teams.value = [
        ...teams.value,
        { id: project.teamId, name: project.teamName },
      ];
    }
  }
  ...
  async function toggleFavoriteProject(projectId: string, isFavorite: boolean) {
    ...
    try {
      ...
      if (project.isDeleted)
        throw new Error("Нельзя добавить удалённый проект в избранное");

      const updatedProject = await apiToggleFavoriteProject(
        projectId,
        isFavorite,
      );
      const index = projects.value.findIndex((p) => p.id === projectId);
  ...
  async function restoreProject(projectId: string) {
    ...
    try {
      ...
      error.value =
        e instanceof Error ? e.message : "Не удалось восстановить проект";
    }
  }
  ...
  async function permanentlyDeleteProject(projectId: string) {
    ...
    try {
      ...
      error.value =
        e instanceof Error ? e.message : "Не удалось удалить проект";
    }
  }
  ...
  async function updateMember(
    ...
    try {
      ...
      const index = currentProjectMembers.value.findIndex(
        (m) => m.id === memberId,
      );
      if (index !== -1) {
  ...
  async function deleteRole(projectId: string, roleId: string) {
    try {
      ...
      currentProjectRoles.value = currentProjectRoles.value.filter(
        (r) => r.id !== roleId,
      );
    } catch {
