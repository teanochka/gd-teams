...
function sanitizePageInPlace(page: LotionPage) {
  ...
  const nextCardBlockIds =
    page.card?.blockIds.filter((blockId) => blockIds.has(blockId)) ?? [];
...
export const useDocumentsStore = defineStore("documents", () => {
  ...
  function setCardBlockIds(documentId: NodeId, blockIds: string[]) {
    ...
    const nextBlockIds = [...new Set(blockIds)].filter((blockId) =>
