...
def main():
    ...
    if not run_command([venv_python, 'manage.py', 'migrate'], cwd=backend_dir):
        print("Failed to apply migrations.")
        return

    # 5. Prepare db.json for migration script
    # migrate_data.py expects db.json in backend/ folder
    source_db = os.path.join(root_dir, 'db.json')
    target_db = os.path.join(backend_dir, 'db.json')
    ...
    if not run_command([venv_python, 'migrate_data.py'], cwd=backend_dir):
